...
public class Horse : Photon.MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (this.State == "mounted")
		{
			...
			if (this.controller.targetDirection != -874f)
			{
				...
				if (this.controller.isWALKDown)
				{
					...
					{
						base.rigidbody.AddForce((0f - this.speed) * 0.6f * base.rigidbody.velocity.normalized, ForceMode.Acceleration);
					}
				}
				else
				{
					...
					{
						base.rigidbody.AddForce((0f - this.speed) * base.rigidbody.velocity.normalized, ForceMode.Acceleration);
					}
				}
		...
		else if (this.State == "follow")
		{
			...
			float num = 0f - Mathf.DeltaAngle(FengMath.getHorizontalAngle(base.transform.position, this.setPoint), base.gameObject.transform.rotation.eulerAngles.y - 90f);
			...
			if (Vector3.Distance(this.setPoint, base.transform.position) < 20f)
			{
				...
				if (base.rigidbody.velocity.magnitude >= this.speed)
				{
					base.rigidbody.AddForce((0f - this.speed) * 0.7f * base.rigidbody.velocity.normalized, ForceMode.Acceleration);
				}
			}
			else
			{
	...
	public void mounted()
	{
		...
		if (this.myHero != null)
		{
			this.myHero.GetComponent<HERO>().SetInterpolationIfEnabled(interpolate: false);
		}
	}
	...
	public void unmounted()
	{
		...
		if (this.myHero != null)
		{
			this.myHero.GetComponent<HERO>().SetInterpolationIfEnabled(interpolate: true);
		}
	}
}
