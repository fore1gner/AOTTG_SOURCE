...
public class UIDrawCall : MonoBehaviour
{
	...
	public bool isClipped => this.mClippedMat != null;
	...
	public void Set(BetterList<Vector3> verts, BetterList<Vector3> norms, BetterList<Vector4> tans, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (size > 0 && size == uvs.size && size == cols.size && size % 4 == 0)
		{
			...
			{
				int num = (size >> 1) * 3;
				bool rebuildIndices = this.mIndices == null || this.mIndices.Length != num;
				if (rebuildIndices)
				{
	...
	private void UpdateMaterials()
	{
		if (this.mClipping != 0)
		{
			...
			if (this.mClipping != 0)
			{
				string text = this.mSharedMat.shader.name.Replace(" (AlphaClip)", string.Empty).Replace(" (SoftClip)", string.Empty);
		...
		if (this.mDepthMat != null)
		{
			...
			{
				this.mRen.sharedMaterials = new Material[2] { this.mDepthMat, material };
			}
		}
		else if (this.mRen.sharedMaterial != material)
		{
			this.mRen.sharedMaterials = new Material[1] { material };
		}
	}
}
