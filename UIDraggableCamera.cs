...
public class UIDraggableCamera : IgnoreTimeScale
{
	...
	private Vector3 CalculateConstrainOffset()
	{
		...
		Vector3 position = new Vector3(this.mCam.rect.xMin * (float)Screen.width, this.mCam.rect.yMin * (float)Screen.height, 0f);
		Vector3 position2 = new Vector3(this.mCam.rect.xMax * (float)Screen.width, this.mCam.rect.yMax * (float)Screen.height, 0f);
		position = this.mCam.ScreenToWorldPoint(position);
		position2 = this.mCam.ScreenToWorldPoint(position2);
	...
	public void Drag(Vector2 delta)
	{
		...
		if (this.dragEffect != UIDragObject.DragEffect.MomentumAndSpring && this.ConstrainToBounds(immediate: true))
		{
	...
	public void Press(bool isPressed)
	{
		...
		{
			this.ConstrainToBounds(immediate: false);
		}
	}
