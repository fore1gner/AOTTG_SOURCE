...
public static class NGUITools
{
	...
	{
		NGUITools.SetActiveSelf(t.gameObject, state: true);
		int i = 0;
		...
		{
			NGUITools.Activate(t.GetChild(j));
		}
	}
	...
	{
		NGUITools.SetActiveSelf(t.gameObject, state: false);
	}
	...
	public static string StripSymbols(string text)
	{
		if (text != null)
		{
			...
			while (num < length)
			{
				if (text[num] == '[')
				{
					int num2 = NGUITools.ParseSymbol(text, num, null, premultiply: false);
					if (num2 > 0)
					{
