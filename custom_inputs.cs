...
public class custom_inputs : MonoBehaviour
{
	...
	private void drawButtons1()
	{
		...
		GUI.Label(new Rect(this.DescriptionBox_X, num - 10f, this.DescriptionSize, this.buttonHeight), "name", "textfield");
		GUI.Label(new Rect(this.InputBox1_X, num - 10f, this.DescriptionSize, this.buttonHeight), "input", "textfield");
		GUI.Label(new Rect(this.InputBox2_X, num - 10f, this.DescriptionSize, this.buttonHeight), "alt input", "textfield");
		for (int i = 0; i < this.DescriptionString.Length; i++)
		{
			num += this.BoxesMargin_Y;
			GUI.Label(new Rect(this.DescriptionBox_X, num, this.DescriptionSize, this.buttonHeight), this.DescriptionString[i], "box");
			Rect position = new Rect(this.InputBox1_X, num, this.buttonSize, this.buttonHeight);
			...
			if (this.inputBool[i])
			{
				GUI.Toggle(position, value: true, string.Empty, this.OurSkin.button);
			}
	...
	private void drawButtons2()
	{
		...
		{
			num += this.BoxesMargin_Y;
			Rect position = new Rect(this.InputBox2_X, num, this.buttonSize, this.buttonHeight);
	...
	private void loadConfig()
	{
		...
		this.joystickString = string2.Split(separator2);
		char[] separator3 = new char[1] { '*' };
		this.inputString = string3.Split(separator3);
		char[] separator4 = new char[1] { '*' };
		string[] array2 = string4.Split(separator4);
		char[] separator5 = new char[1] { '*' };
		this.joystickString2 = string5.Split(separator5);
		char[] separator6 = new char[1] { '*' };
		this.inputString2 = string6.Split(separator6);
		for (int i = 0; i < this.DescriptionString.Length; i++)
		{
			int.TryParse(array[i], out var result);
			this.inputKey[i] = (KeyCode)result;
			int.TryParse(array2[i], out var result2);
			this.inputKey2[i] = (KeyCode)result2;
	...
	private void reset2defaults()
	{
		...
		for (int num = this.DescriptionString.Length - 1; num > -1; num--)
		{
			int num2 = (int)this.default_inputKeys[num];
			text = num2 + "*" + text;
			text2 += "#*";
			text3 = this.default_inputKeys[num].ToString() + "*" + text3;
			PlayerPrefs.SetString("KeyCodes", text);
			PlayerPrefs.SetString("Joystick_input", text2);
			PlayerPrefs.SetString("Names_input", text3);
			num2 = (int)this.alt_default_inputKeys[num];
			text4 = num2 + "*" + text4;
			text5 += "#*";
			text6 = this.alt_default_inputKeys[num].ToString() + "*" + text6;
	...
	private void saveInputs()
	{
		...
		for (int num = this.DescriptionString.Length - 1; num > -1; num--)
		{
			int num2 = (int)this.inputKey[num];
			text = num2 + "*" + text;
			text2 = this.joystickString[num] + "*" + text2;
			text3 = this.inputString[num] + "*" + text3;
			num2 = (int)this.inputKey2[num];
			text4 = num2 + "*" + text4;
			text5 = this.joystickString2[num] + "*" + text5;
			text6 = this.inputString2[num] + "*" + text6;
		}
		PlayerPrefs.SetString("KeyCodes", text);
		PlayerPrefs.SetString("Joystick_input", text2);
		PlayerPrefs.SetString("Names_input", text3);
