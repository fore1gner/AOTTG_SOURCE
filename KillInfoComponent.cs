...
public class KillInfoComponent : MonoBehaviour
{
	...
	public void moveOn()
	{
		...
		this.groupBig.SetActive(value: false);
		this.groupSmall.SetActive(value: true);
	}
	...
	{
		this.groupBig.SetActive(value: true);
		this.groupSmall.SetActive(value: true);
		if (!isTitan1)
		{
			this.leftTitan.SetActive(value: false);
			this.spriteSkeleton.SetActive(value: false);
			this.sleftTitan.SetActive(value: false);
			this.sspriteSkeleton.SetActive(value: false);
			this.labelNameLeft.transform.position += new Vector3(18f, 0f, 0f);
			this.slabelNameLeft.transform.position += new Vector3(16f, 0f, 0f);
		}
		else
		{
			this.spriteSword.SetActive(value: false);
			this.sspriteSword.SetActive(value: false);
		...
		if (!isTitan2)
		{
			this.rightTitan.SetActive(value: false);
			this.srightTitan.SetActive(value: false);
		}
		...
		this.groupSmall.SetActive(value: false);
	}
	...
	private void Update()
	{
		if (this.start)
		{
			...
			{
				float num = (int)(100f - (float)Screen.height * 0.5f) + this.col * this.offset;
